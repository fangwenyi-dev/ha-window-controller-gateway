...
| ctype | 方向 | 说明 | HA 响应 |
|-------|------|------|---------|
| 001 | 网关→HA | 绑定网关 | HA 回复 001(errcode=0, uuid) |
| 002 | 网关→HA | 网关状态上报 / 设备列表 | HA 回复 002(errcode=0) |
| 003 | HA→网关 | 配对(bind=1) / 解绑(bind=0) 子设备 | 网关回复 003(errcode=0, sn) |
| 004 | HA→网关 | 设备控制（开/关/停/内倒/风锁模式） | 网关回复 004(errcode=0) |
| 005 | 网关→HA | 设备状态上报 | HA 回复 005(errcode=0) |
| 006 | 网关→HA | 批量设备状态上报 | HA 回复 006(errcode=0) |
| 007 | 网关→HA | 设备事件上报 | HA 回复 007(errcode=0) |
| 008 | 网关→HA | 网关配置更新 | HA 回复 008(errcode=0) |
| 009 | 网关→HA | 设备配置更新 | HA 回复 009(errcode=0) |
| 010 | 网关→HA | 系统消息 | HA 回复 010(errcode=0) |

> **命令重发机制**：HA 主动下发的 003/004 命令，如果 5 秒内未收到网关回复，自动重发 1 次（共发送 2 次），仍未收到则放弃并记录警告日志。
...
- **命令重发**：003/004 命令 5 秒未收到回复自动重发 1 次（`COMMAND_ACK_TIMEOUT=5`，`COMMAND_MAX_RETRIES=2`）
...
- 新增: 003/004 命令重发机制 — 网关 5 秒未回复自动重发 1 次，收到回复取消定时器
- 新增: 解绑命令也加入重发机制，通过 command_id 精确匹配回复
- 优化: 移除 HA 主动发送 001/002/005 的无效代码（这些是网关主动发起，HA 发了网关不响应）
- 优化: 协议响应规范化 — 001/002/005 由 HA 回复一次确认，003/004 网关回复后 HA 不再回复
