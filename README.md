...
| 003 | HA 主动发起 | 003 | 配对(bind=1) / 解绑(bind=0) 子设备，网关响应，未响应最多重发 1 次 |
| 004 | HA 主动发起 | 004 | 设备控制（开/关/停/内倒/风锁模式），网关响应，未响应最多重发 1 次 |
| 005 | 网关主动发起 | 005 | 设备状态上报，HA 响应一次 |
| 006 | HA 主动发起 | 006 | 网关响应，未响应最多重发 1 次 |
| 007 | HA 主动发起 | 007 | 网关响应，未响应最多重发 1 次 |

> **命令重发机制**：HA 主动下发的 003/004/006/007 命令，如果 5 秒内未收到网关回复，自动重发 1 次（共发送 2 次），仍未收到则放弃并记录警告日志。
...
- **命令重发**：003/004/006/007 命令 5 秒未收到回复自动重发 1 次（`COMMAND_ACK_TIMEOUT=5`，`COMMAND_MAX_RETRIES=2`）
...
- 修复: 003 配对（bind=1）不重发（用户主动操作，避免重复配对）；003 解绑（bind=0）/004/006/007 保留"未响应最多重发一次"
