    ...
    async def on_device_removed(device_sn: str, device_name: str, device_type: str):
        ...
        if device_type == DEVICE_TYPE_WINDOW_OPENER:
            if device_sn in created_covers:
                # 获取实体
                entity = created_covers[device_sn]
                # 从跟踪字典中删除
                del created_covers[device_sn]
                _LOGGER.info("已清理设备 %s 的开窗器实体跟踪", device_name)
                ...
                try:
                    ...
                    if entity_id:
                        ...
                        _LOGGER.info("已从实体注册表中删除设备 %s 的实体", device_name)
                except Exception as e:
                    _LOGGER.error("从实体注册表中删除设备 %s 的实体失败: %s", device_name, e)
