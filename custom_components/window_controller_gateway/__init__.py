...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    try:
        ...
        # 监听设备删除事件
        async def async_handle_device_remove(event):
            """处理设备删除事件"""
                ...
                if device:
                    ...
                    for identifier in device.identifiers:
                        if identifier[0] == DOMAIN:
                            ...
                            # 检查是否是子设备（不是网关本身）
                            if device_sn != gateway_sn:
                                # 从设备管理器中删除设备
                                entry_data = hass.data[DOMAIN].get(entry.entry_id)
                                if entry_data:
                                    device_manager = entry_data.get("device_manager")
                                    if device_manager:
                                        await device_manager.remove_device(device_sn)
                                        _LOGGER.info("设备已从集成中删除: %s", device_sn)
                            break
            except Exception as e:
                _LOGGER.error("处理设备删除事件失败: %s", e)
        
        # 监听设备删除事件 - 使用直接的事件名称字符串
        EVENT_DEVICE_REMOVED = "device_removed"
        hass.bus.async_listen(EVENT_DEVICE_REMOVED, async_handle_device_remove)
...
async def async_unload_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    if DOMAIN not in hass.data or entry_id not in hass.data[DOMAIN]:
        _LOGGER.warning("要卸载的条目 %s 未在数据中找到", entry_id)
        return False
