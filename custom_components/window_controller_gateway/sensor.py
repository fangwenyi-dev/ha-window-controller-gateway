...
async def async_setup_entry(
    ...
    async def on_device_removed(device_sn: str, device_name: str, device_type: str):
        ...
        if device_sn in created_sensors:
            ...
            # 移除电池传感器
            battery_sensor = sensors.get("battery")
            if battery_sensor:
                try:
                    await battery_sensor.async_remove()
                    _LOGGER.info("已从Home Assistant中移除设备 %s 的电池传感器", device_name)
                except Exception as e:
                    _LOGGER.error("移除电池传感器失败: %s", e)
            # 移除状态传感器
            status_sensor = sensors.get("status")
            if status_sensor:
                try:
                    await status_sensor.async_remove()
                    _LOGGER.info("已从Home Assistant中移除设备 %s 的状态传感器", device_name)
                except Exception as e:
                    _LOGGER.error("移除状态传感器失败: %s", e)
            # 从跟踪字典中删除
            del created_sensors[device_sn]
            _LOGGER.info("已清理设备 %s 的传感器实体", device_name)
