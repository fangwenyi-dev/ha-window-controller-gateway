...
class WindowControllerOpenButton(ButtonEntity):
    ...
    def __init__(
        ...
        self.device_sn = device_sn
        self.device_name = device_name
        self._attr_name = f"{device_name} 开"
...
class WindowControllerCloseButton(ButtonEntity):
    ...
    def __init__(
        ...
        self.device_sn = device_sn
        self.device_name = device_name
        self._attr_name = f"{device_name} 关"
...
class WindowControllerStopButton(ButtonEntity):
    ...
    def __init__(
        ...
        self.device_sn = device_sn
        self.device_name = device_name
        self._attr_name = f"{device_name} 停"
    ...
    async def on_device_added(device_sn: str, device_name: str, device_type: str):
        ...
        if device_type == DEVICE_TYPE_WINDOW_OPENER:
            ...
            if not existing_entity:
                ...
                async_add_entities([remove_button, a_button, open_button, close_button, stop_button])
                created_remove_buttons[device_sn] = remove_button
                # 更新entry_data中的删除按钮跟踪信息
                entry_data["created_remove_buttons"] = created_remove_buttons
                _LOGGER.info("自动为设备 %s 添加删除按钮、A按钮、打开按钮、关闭按钮和停止按钮", device_name)
            else:
                # 删除按钮已经存在，只创建A按钮、打开按钮、关闭按钮和停止按钮
                a_button = WindowControllerAButton(
                    hass,
                    device_manager,
                    mqtt_handler,
                ...
                async_add_entities([a_button, open_button, close_button, stop_button])
                _LOGGER.info("设备 %s 的删除按钮已存在，只添加A按钮、打开按钮、关闭按钮和停止按钮", device_name)
