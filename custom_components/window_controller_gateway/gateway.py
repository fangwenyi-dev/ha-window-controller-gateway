...
class GatewayDeviceRemoveButton(ButtonEntity):
    ...
    def __init__(
        ...
        self._attr_name = f"{device_name} 删除"
        # 添加时间戳确保唯一ID
        timestamp = int(time.time() * 1000)
        self._attr_unique_id = f"{ENTITY_GATEWAY_PREFIX}{gateway_sn}_remove_{device_sn}_{timestamp}"
    ...
    async def async_press(self) -> None:
        ...
        try:
            ...
            _LOGGER.info("已触发设备 %s 的解绑模式", self.device_sn)
        except Exception as e:
