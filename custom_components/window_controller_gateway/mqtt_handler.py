...
class WindowControllerMQTTHandler:
        ...
        def handle_gateway_response(msg):
            ...
            try:
                ...
                if "head" in payload and "ctype" in payload:
                    ...
                    # 根据不同的消息类型调用相应的处理函数
                    if ctype == "001":
                        self.hass.create_task(self._handle_ctype_001(payload, ctype, data))
                    elif ctype == "002":
                        self.hass.create_task(self._handle_ctype_002(payload, ctype, data))
                    elif ctype == "003":
                        self.hass.create_task(self._handle_ctype_003(payload, ctype, data))
                    elif ctype == "004":
                        self.hass.create_task(self._handle_ctype_004(payload, ctype, data))
                    elif ctype == "005":
                        self.hass.create_task(self._handle_ctype_005(payload, ctype, data))
                    elif ctype == "006":
                        self.hass.create_task(self._handle_ctype_006(payload, ctype, data))
                    elif ctype == "007":
                        self.hass.create_task(self._handle_ctype_007(payload, ctype, data))
                    elif ctype == "008":
                        self.hass.create_task(self._handle_ctype_008(payload, ctype, data))
                    elif ctype == "009":
                        self.hass.create_task(self._handle_ctype_009(payload, ctype, data))
                    elif ctype == "010":
                        self.hass.create_task(self._handle_ctype_010(payload, ctype, data))
                    else:
    ...
    async def _reconnect_mqtt(self):
        ...
        while retry_count < max_retries:
            try:
                _LOGGER.info("尝试重新连接MQTT... (重试 %d/%d)", retry_count + 1, max_retries)
                ...
                _LOGGER.info("MQTT重新连接成功")
                return
            except Exception as e:
                retry_count += 1
                _LOGGER.error("MQTT重连失败: %s", e)
                
                if retry_count < max_retries:
                    # 指数退避策略
                    delay = retry_delay * (2 ** (retry_count - 1))
                    _LOGGER.info("%d秒后重试...", delay)
                    await asyncio.sleep(delay)
                else:
                    _LOGGER.error("MQTT重连失败，已达到最大重试次数")
                    # 标记为离线
                    if self.connected:
                        self.connected = False
                        self._notify_status_change()
    ...
    async def send_command(self, device_sn: str, command: str, params: Optional[Dict[str, Any]] = None):
        ...
        # 递增ID，保持在合理范围内
        self.command_id += 1
        if self.command_id > 999999:  # 保持ID在合理范围内
            self.command_id = 1
        ...
        try:
            ...
            _LOGGER.error("发送MQTT命令失败: %s\n命令: %s\n设备: %s\n主题: %s\n载荷: %s", e, command, device_sn, self.TOPIC_GATEWAY_REQ, payload)
    ...
    def remove_status_callback(self, *args: Union[str, Callable[[Union[str, Dict[str, Any]], Any], None]]):
        ...
        if len(args) == 2:
            ...
            if "gateway" in self._status_callbacks:
                ...
                # 如果网关没有回调了，清理网关条目
                if not self._status_callbacks["gateway"]:
        ...
        try:
            ...
            if "devices" in data:
                ...
                if add_tasks:
                    batch_size = 10
                    total_success = 0
                    for i in range(0, len(add_tasks), batch_size):
                        batch_tasks = add_tasks[i:i+batch_size]
                        results = await asyncio.gather(*batch_tasks, return_exceptions=True)
                        success_count = sum(1 for r in results if not isinstance(r, Exception))
                        total_success += success_count
                        _LOGGER.info("批量添加设备完成，批次: %d，成功: %d，总数: %d", 
                                   i//batch_size + 1, success_count, len(batch_tasks))
                    _LOGGER.info("所有批次添加完成，总成功: %d，总总数: %d", total_success, len(add_tasks))
                ...
                if update_tasks:
                    ...
                    for i in range(0, len(update_tasks), batch_size):
                        batch_tasks = update_tasks[i:i+batch_size]
                        results = await asyncio.gather(*batch_tasks, return_exceptions=True)
                        success_count = sum(1 for r in results if not isinstance(r, Exception))
                        total_success += success_count
                        _LOGGER.info("批量更新设备状态完成，批次: %d，成功: %d，总数: %d", 
                                   i//batch_size + 1, success_count, len(batch_tasks))
                    _LOGGER.info("所有批次更新完成，总成功: %d，总总数: %d", total_success, len(update_tasks))
    ...
    async def _quick_add_device(self, device_sn, device_info):
        ...
        device_name = f"开窗器 {device_count + 1:02d}"
    ...
    async def _handle_ctype_003(self, payload, ctype, data):
        ...
        if errcode == 0 and device_sn:
            ...
            self.hass.create_task(
                self.device_manager.add_device(device_sn, device_name, DEVICE_TYPE_WINDOW_OPENER, force=True)
            )
