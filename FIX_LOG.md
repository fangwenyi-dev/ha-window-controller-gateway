...
- **文件**: __init__.py async_unload_entry / async_remove_entry
- **问题**: 
  - 卸载配置条目时未先保存持久化数据
...
## Fix 6 - __init__.py - 停止事件监听器泄漏 + entry_data 未定义（V2 Bug 1）
- **文件**: __init__.py 第 601-612 行
- **问题**: 
  - async_listen_once 返回值未保存，中途卸载后 HA 停机会重复卸载
  - entry_data["_stop_unsub"] 引用未定义变量（V2 引入的新 Bug）
- **修复**: 
  - 保存 _stop_unsub，在 async_unload_entry 中主动取消监听
  - 将内联 dict 改为 entry_data 变量先赋值再写入 hass.data
- **回滚**: 删除 _stop_unsub 存储和取消逻辑，恢复内联 dict
...
## Fix 9 - gateway.py - 配对按钮绕过命令管理器（V2 修复：空参数 Bug）
- **文件**: gateway.py 第 152 行
- **问题**: 
  - V1: GatewayPairingButton 直接使用 mqtt.async_publish，绕过 send_command
  - V2: send_command("", "start_pairing") 空 device_sn 导致 if not device_sn 短路返回 False
- **修复**: 
  - V1: 使用 mqtt_handler.send_command 统一命令管理
  - V2: 改为 send_command(self.gateway_sn, "start_pairing") 传入网关 SN
...
# V2 新增 Bug 修复（第二轮代码审查后）

## Bug V2-3 - 迁移后传感器回调未更新
- **文件**: __init__.py 第 395-405 行、第 625 行
- **问题**: 
  - 自动迁移时 remove_old_gateway 可能为 False，旧网关保留但其传感器回调绑定的 MQTT 处理器不再接收设备消息
  - 手动迁移时 remove_old_gateway=False 时旧实体残留
- **修复**: 
  - 自动迁移（async_setup_entry 中）：强制 remove_old_gateway=True（替换流程必然移除旧网关）
  - 手动迁移（服务调用）：remove_old_gateway=False 时重载旧网关平台以清理旧实体
- **回滚**: 恢复自动迁移使用 migration_info 中的 remove_old_gateway 值，删除 else 分支的重载逻辑

## Bug V2-4 - 测试连接时订阅泄漏
- **文件**: config_flow.py 第 290-334 行
- **问题**: _test_gateway_connectivity 中 setup() 失败或发生异常时返回但不调用 cleanup()
- **修复**: 改用 try/finally 确保 mqtt_handler.cleanup() 在任何路径下都被调用
- **回滚**: 恢复原 try/except 结构，删除 finally